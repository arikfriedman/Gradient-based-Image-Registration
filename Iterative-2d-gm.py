...
if __name__ == "__main__":
    ...
    min_height = int(np.minimum(height1, height2))
    min_width = int(np.minimum(width1, width2))
    ...
    for epoch in range(epochs):
        ...
        if d_total[0] >= 0:
            I1 = original_I1[:, int(np.ceil(d_total[0])):min_width]
        else:
            I1 = original_I1[:, 0:min_width+int(np.floor(d_total[0]))]
        if d_total[1] >= 0:
            I1 = I1[int(np.ceil(d_total[1])):min_height, :]
        else:
            I1 = I1[0:min_height+int(np.floor(d_total[1])), :]
    ...
    print(str(round(d_total[0], 2)) + " " + str(round(d_total[1], 2)))
